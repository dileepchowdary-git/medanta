WITH base AS (
    SELECT
        ...
        -- ADDED: Select the latest reason_fk and reason text
        ass.latest_reason_fk AS Reason_ID,
    ...
    -- ADDED: Join to get the latest AI status reason
    LEFT JOIN (
        SELECT
            study_fk,
            argMax(reason_fk, created_at) AS latest_reason_fk
        FROM AIStudyStatuses
        GROUP BY study_fk
    ) AS ass ON ass.study_fk = s.id
    -- ADDED: Join to get the reason text
    LEFT JOIN AISkipReasons AS asr ON asr.id = ass.latest_reason_fk
